...
// spawn wheel
private _newobj = ["FL_parts_controlsurfaces", getPosASL _unit, _hitPointDamage] call ace_repair_fnc_spawnObject;
TRACE_2("new controlsurfaces created",_newobj,damage _newobj);
...
if (ACEVAR(repair,DisplayTextOnRepair)) then {
    [localize LSTRING(RemovedControlsurfaces)] call ace_common_fnc_displayTextStructured;
};
