...
// spawn wheel
private _newobj = ["FL_parts_avionics", getPosASL _unit, _hitPointDamage] call ace_repair_fnc_spawnObject;
TRACE_2("new avionics created",_newobj,damage _newobj);
...
if (ACEVAR(repair,DisplayTextOnRepair)) then {
    [localize LSTRING(RemovedAvionics)] call ace_common_fnc_displayTextStructured;
};
