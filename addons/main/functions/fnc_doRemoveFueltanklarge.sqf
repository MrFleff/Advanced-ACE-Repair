...
// spawn wheel
private _newobj = ["FL_parts_fueltanklarge", getPosASL _unit, _hitPointDamage] call ace_repair_fnc_spawnObject;
TRACE_2("new fuel tank large created",_newobj,damage _newobj);
...
if (ACEVAR(repair,DisplayTextOnRepair)) then {
    [localize LSTRING(RemovedFueltanklarge)] call ace_common_fnc_displayTextStructured;
};
