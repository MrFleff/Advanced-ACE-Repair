...
private _condition = {[_this select 1, _this select 0, "", "AdvMajorRepair"] call DEFUNC(repair,canRepair)};
private _statement = {[_this select 1, _this select 0, "", "AdvMajorRepair"] call DEFUNC(repair,repair)};
private _action = ["AdvMajorRepair", _text2, _icon, _statement, _condition, {}, [], "", 4] call EFUNC(interact_menu,createAction);
[_type, 0, ["ACE_MainActions"], _action] call EFUNC(interact_menu,addActionToClass);

_initializedClasses pushBack _type;
SETMVAR(GVAR(initializedClasses),_initializedClasses);  20