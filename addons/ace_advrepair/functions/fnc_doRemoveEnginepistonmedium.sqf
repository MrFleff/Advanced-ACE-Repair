...
// spawn wheel
private _newobj = ["FL_parts_enginepistonmedium", getPosASL _unit, _hitPointDamage] call EFUNC(repair,spawnObject);
...
if (EGVAR(repair,DisplayTextOnRepair)) then {
    [localize LSTRING(RemovedEnginepistonmedium)] call EFUNC(common,displayTextStructured);
};
