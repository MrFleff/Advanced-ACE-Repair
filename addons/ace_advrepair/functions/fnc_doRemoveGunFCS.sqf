...
// spawn wheel
private _newobj = ["FL_parts_gunfcs", getPosASL _unit, _hitPointDamage] call EFUNC(repair,spawnObject);
...
if (EGVAR(repair,DisplayTextOnRepair)) then {
    [localize LSTRING(RemovedGunFCS)] call EFUNC(common,displayTextStructured);
};
