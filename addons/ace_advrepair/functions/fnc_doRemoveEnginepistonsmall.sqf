...
// spawn wheel
private _newobj = ["FL_parts_enginepistonsmall", getPosASL _unit, _hitPointDamage] call EFUNC(repair,spawnObject);
...
if (EGVAR(repair,DisplayTextOnRepair)) then {
    [localize LSTRING(RemovedEnginepistonsmall)] call EFUNC(common,displayTextStructured);
};
