...
// spawn wheel
private _newobj = ["FL_parts_fueltanklarge", getPosASL _unit, _hitPointDamage] call EFUNC(repair,spawnObject);
...
if (EGVAR(repair,DisplayTextOnRepair)) then {
    [localize LSTRING(RemovedFueltanklarge)] call EFUNC(common,displayTextStructured);
};
