...
// spawn wheel
private _newobj = ["FL_parts_engineturbinelarge", getPosASL _unit, _hitPointDamage] call EFUNC(repair,spawnObject);
...
if (EGVAR(repair,DisplayTextOnRepair)) then {
    [localize LSTRING(RemovedEngineturbinelarge)] call EFUNC(common,displayTextStructured);
};
