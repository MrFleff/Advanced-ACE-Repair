...
// spawn wheel
private _newobj = ["FL_parts_fueltanksmall", getPosASL _unit, _hitPointDamage] call EFUNC(repair,spawnObject);
...
if (EGVAR(repair,DisplayTextOnRepair)) then {
    [localize LSTRING(RemovedFueltanksmall)] call EFUNC(common,displayTextStructured);
};
