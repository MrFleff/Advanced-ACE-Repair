...
// spawn wheel
private _newobj = ["FL_parts_engineturbinesmall", getPosASL _unit, _hitPointDamage] call EFUNC(repair,spawnObject);
...
if (EGVAR(repair,DisplayTextOnRepair)) then {
    [localize LSTRING(RemovedEngineturbinesmall)] call EFUNC(common,displayTextStructured);
};
