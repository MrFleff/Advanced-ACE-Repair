...
// spawn wheel
private _newobj = ["FL_parts_rotorassembly", getPosASL _unit, _hitPointDamage] call EFUNC(repair,spawnObject);
...
if (EGVAR(repair,DisplayTextOnRepair)) then {
    [localize LSTRING(RemovedRotorAssembly)] call EFUNC(common,displayTextStructured);
};
